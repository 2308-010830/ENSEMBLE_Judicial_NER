...
class Embeddings(object):
    ...
    def make_embeddings_simple_in_memory(self, name="fasttext-crawl"):
        ...
        if description is not None:
            ...
            if self.extension == 'bin':
                ...
                with open(embeddings_path, encoding='utf-16') as f:
                    for line in f:
                        line = line.strip()
                        line = line.split(' ')
                        if begin:
    ...
    def load_embeddings_from_file(self, embeddings_path):
        ...
        for line in tqdm(embedding_file, total=nb_lines):
            ...
            if len(word.encode(encoding='UTF-16')) < self.env.max_key_size():   
                txn.put(word.encode(encoding='UTF-16'), _serialize_pickle(vector))  
    ...
    def get_word_vector(self, word):
        ...
        try:    
            with self.env.begin() as txn:
                vector = txn.get(word.encode(encoding='UTF-16'))
                if vector:
        ...
        try:    
            ...
            with self.env_ELMo.begin() as txn:
                for i in range(0, len(token_list)):
                    ...
                    vector = txn.get(the_hash.encode(encoding='UTF-16'))
                    if vector:
    ...
    def cache_ELMo_lmdb_vector(self, token_list, ELMo_vector):
        ...
        for i in range(0, len(token_list)):
            # get a hash for the token_list
            the_hash = list_digest(token_list[i])
            txn.put(the_hash.encode(encoding='UTF-16'), _serialize_pickle(ELMo_vector[i]))  
        txn.commit()
    ...
    def cache_BERT_lmdb_vector(self, sentence, BERT_vector):
        ...
        # get a hash for the token_list
        the_hash = list_digest(sentence)
        txn.put(the_hash.encode(encoding='UTF-16'), _serialize_pickle(BERT_vector))  
        txn.commit()
...
def open_embedding_file(embeddings_path):
    ...
    if embeddings_path.endswith(".gz"):
        ...
        embedding_file = gzip.open(embeddings_path, mode="r")#, encoding='utf-16')
    ...
    elif embeddings_path.endswith(".hdf5"):
        print('========yes')
        import h5py
        embedding_file = h5py.File(embeddings_path, "r")
    else:
...
def list_digest(strings):
    ...
    for s in strings:
        ...
        hash.update(s.encode(encoding='UTF-16'))
